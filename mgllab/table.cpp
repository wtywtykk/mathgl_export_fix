...
public:
	...
	void setup(const char *desc, const char *ch=NULL)
	{
		...
		{	wch->label(mgl_gettext("not used"));
			wch->deactivate();	}
	}
	void init()	{	OK=false;	}
	XYZDlg()
	{
		w = new Fl_Double_Window(325, 125, mgl_gettext("Change data sizes"));
		box = new Fl_Box(10, 10, 305, 40);
		box->align(FL_ALIGN_LEFT|FL_ALIGN_INSIDE);
		wmx = new Fl_Spinner(30, 55, 75, 25, "mx");
		wmx->tooltip(mgl_gettext("New size of data on 1st dimension (x-direction)"));
		wmy = new Fl_Spinner(135, 55, 75, 25, "my");
		wmy->tooltip(mgl_gettext("New size of data on 2nd dimension (y-direction)"));
		wmz = new Fl_Spinner(240, 55, 75, 25, "mz");
		wmz->tooltip(mgl_gettext("New size of data on 3d dimension (z-direction)"));
		wch = new Fl_Check_Button(15, 90, 95, 25);
		Fl_Button *o = new Fl_Button(125, 90, 85, 25, mgl_gettext("Cancel"));
		o->tooltip(mgl_gettext("Do nothing and close window"));	o->callback(cb_dlg_cancel,this);
		o = new Fl_Return_Button(230, 90, 85, 25, mgl_gettext("Change"));
		o->tooltip(mgl_gettext("Change (resize) data"));	o->callback(cb_dlg_ok,this);
		w->end();	w->set_modal();
	}
...
{
	TableWindow* e = (TableWindow*)v;
	xyz_dlg.setup(mgl_gettext("Specify new data size\nData will be zero filled"));
	if(xyz_dlg.run())
	{
		HMDT d = dynamic_cast<HMDT>(e->var);
		if(d)
...
{
	TableWindow* e = (TableWindow*)v;
	xyz_dlg.setup(mgl_gettext("Specify the skiping step\nEach m-th point will be skiped"), mgl_gettext("smoothed"));	if(xyz_dlg.run())
	{
		HMDT d = dynamic_cast<HMDT>(e->var);
		if(d)
...
public:
	...
	void cb_ok()
	{
		...
		if(result.empty())
		{	fl_alert(mgl_gettext("You need to specify direction(s)"));	return;	}
...
public:
	...
	void cb_ok()
	{
		...
		if(!s || !s[0])
		{	fl_alert(mgl_gettext("You need to provide output name"));	return;	}
...
void load_dat_cb(Fl_Widget*, void*v)
{
	...
	if(newfile)
	{
		...
		{	// TODO add dialog with choice of HDF names
			const char *name = fl_input(mgl_gettext("Enter name of data"),"");
			if(name)
			{
...
void imp_dat_cb(Fl_Widget*, void*v)
{
	...
	if(!d)	fl_alert(mgl_gettext("This operation is not supported for this kind of data."));
}
//-----------------------------------------------------------------------------
...
public:
	NrmDlg()
	{
		...
		wmin = new Fl_Value_Input(10, 25, 115, 25, mgl_gettext("Minimal value (v1)"));
		wmin->align(FL_ALIGN_TOP_LEFT);
		wmin->tooltip(mgl_gettext("Minimal value for resulting data values"));
		wmax = new Fl_Value_Input(10, 70, 115, 25, mgl_gettext("Maximal value (v2)"));
		wmax->align(FL_ALIGN_TOP_LEFT);
		wmax->tooltip(mgl_gettext("Maximal value for resulting data values"));
		...
		o = new Fl_Return_Button(25, 180, 85, 25, mgl_gettext("Change"));	o->callback(cb_dlg_ok,this);
...
{
	TableWindow* e = (TableWindow*)v;
	nrm_dlg.run(mgl_gettext("Fill in range"));
	if(nrm_dlg.ok())
	{
		HMDT d = dynamic_cast<HMDT>(e->var);
		HADT c = dynamic_cast<HADT>(e->var);
...
struct CropDlg : public GeneralDlg
{
	...
	CropDlg()
	{
		w = new Fl_Double_Window(230, 155, mgl_gettext("Crop data"));
		x1 = new Fl_Spinner(45, 25, 80, 25, mgl_gettext("Lower bound"));	x1->align(FL_ALIGN_TOP);
		x2 = new Fl_Spinner(140, 25, 80, 25, mgl_gettext("Upper bound"));	x2->align(FL_ALIGN_TOP);
		...
		Fl_Button *o;
		o = new Fl_Button(45, 120, 75, 25, mgl_gettext("Cancel"));	o->callback(cb_dlg_cancel,this);
		o->tooltip(mgl_gettext("Do nothing and close this window"));
		o = new Fl_Return_Button(145, 120, 75, 25, mgl_gettext("Crop"));	o->callback(cb_dlg_ok,this);
		o->tooltip(mgl_gettext("Change data values and close this window"));
		w->set_modal();	w->end();
	}
	void init()
...
public:
	TrspDlg()
	{
		...
		w = new Fl_Double_Window(200, 90, mgl_gettext("Transpose data"));
		how = new Fl_Choice(10, 20, 180, 25, mgl_gettext("New order of dimensions"));
		how->align(FL_ALIGN_TOP_LEFT);	how->copy(k);	how->value(0);
		Fl_Button *o;
		o = new Fl_Button(30, 55, 75, 25, mgl_gettext("Cancel"));	o->callback(cb_dlg_cancel,this);
		o->tooltip(mgl_gettext("Do nothing and close this window"));
		o = new Fl_Return_Button(115, 55, 75, 25, mgl_gettext("Do"));	o->callback(cb_dlg_ok,this);
		o->tooltip(mgl_gettext("Change data values and close this window"));
		w->set_modal();	w->end();
	}
	void cb_ok()
...
TableWindow::TableWindow(ScriptWindow *e)
{
	...
	o->callback(plot_dat_cb,this);		o->tooltip(mgl_gettext("Data info and preview"));
