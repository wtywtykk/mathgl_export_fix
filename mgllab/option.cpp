...
{
	Fl_Button *o;
	wnd = new Fl_Window(490, 180, mgl_gettext("Command options"));
	new Fl_Box(10, 15, 75, 25, mgl_gettext("X-Range"));
	xmin = new Fl_Input(85, 15, 75, 25);
	xmin->tooltip(mgl_gettext("Minimal value of X for cutting or for coordinate filling"));
	xmax = new Fl_Input(165, 15, 75, 25);
	xmax->tooltip(mgl_gettext("Maximal value of X for cutting or for coordinate filling"));
	new Fl_Box(245, 15, 75, 25, mgl_gettext("Y-Range"));
	ymin = new Fl_Input(320, 15, 75, 25);
	ymin->tooltip(mgl_gettext("Minimal value of Y for cutting or for coordinate filling"));
	ymax = new Fl_Input(400, 15, 75, 25);
	ymax->tooltip(mgl_gettext("Maximal value of Y for cutting or for coordinate filling"));
	new Fl_Box(10, 45, 75, 25, mgl_gettext("Z-Range"));
	zmin = new Fl_Input(85, 45, 75, 25);
	zmin->tooltip(mgl_gettext("Minimal value of Z for cutting or for coordinate filling"));
	zmax = new Fl_Input(165, 45, 75, 25);
	zmax->tooltip(mgl_gettext("Maximal value of Z for cutting or for coordinate filling"));
	new Fl_Box(245, 45, 75, 25, mgl_gettext("C-Range"));
	cmin = new Fl_Input(320, 45, 75, 25);
	cmin->tooltip(mgl_gettext("Low border for determining color or alpha"));
	cmax = new Fl_Input(400, 45, 75, 25);
	cmax->tooltip(mgl_gettext("Upper border for determining color or alpha"));
	...
	amb = new Fl_Input(110, 105, 75, 25, mgl_gettext("Ambient"));	amb->align(FL_ALIGN_TOP);
	amb->tooltip(mgl_gettext("Own brightness of the surface"));
	mesh = new Fl_Input(195, 105, 75, 25, mgl_gettext("Mesh Num"));	mesh->align(FL_ALIGN_TOP);
	mesh->tooltip(mgl_gettext("Approximate number of mesh lines in plot"));
	font = new Fl_Input(280, 105, 75, 25, mgl_gettext("Font Size"));	font->align(FL_ALIGN_TOP);
	font->tooltip(mgl_gettext("Act as default value for font size"));
	cut = new Fl_Choice(365, 105, 75, 25, mgl_gettext("Cutting"));	cut->align(FL_ALIGN_TOP);
	cut->add(mgl_gettext("on"));	cut->add(mgl_gettext("off"));
...
//	as->tooltip(mgl_gettext("Type of arrow at first point of line or curve"));
	dash = new Fl_Choice(110, 50, 80, 25, mgl_gettext("Dashing"));
	dash->align(FL_ALIGN_TOP);	dash->copy(dashing);dash->callback(style_set_cb);
//	dash->tooltip(mgl_gettext("Type dashing for line or curve"));
	ae = new Fl_Choice(210, 50, 80, 25, mgl_gettext("Arrow at end"));
	ae->align(FL_ALIGN_TOP);	ae->copy(arrows);	ae->callback(style_set_cb);
//	ae->tooltip(mgl_gettext("Type of arrow at last point of line or curve"));
	cl = new Fl_Choice(110, 85, 80, 25, mgl_gettext("Color"));	cl->copy(colors);
	cl->callback(style_set_cb);
	mark = new Fl_Choice(110, 120, 80, 25, mgl_gettext("Marks"));
	mark->copy(markers);	mark->callback(style_set_cb);
//	mark->tooltip(mgl_gettext("Type of marks at positions of data points"));
	lw = new Fl_Spinner(110, 155, 80, 25, mgl_gettext("Line width"));
	lw->range(0,9);	lw->step(1);	lw->callback(style_set_cb);
//	lw->tooltip(mgl_gettext("Relative width of line or curve"));
	ltab->end();

	stab = new Fl_Group(0, 25, 295, 230, mgl_gettext("Color scheme"));	stab->hide();
	c[0] = new Fl_Choice(15, 45, 75, 25, mgl_gettext("Color order"));
	...
	d = new Fl_Check_Button(100, 45, 180, 25, mgl_gettext("Colors along coordinates"));
//	w->tooltip(mgl_gettext("Set face color proportional to its position"));
	d->callback(style_set_cb);
	w = new Fl_Check_Button(100, 75, 180, 25, mgl_gettext("Wire or mesh plot"));
	w->callback(style_set_cb);
//	w->tooltip(mgl_gettext("Switch to draw wire isosurface or set to draw mesh on surface"));
	dir = new Fl_Choice(210, 105, 75, 25, mgl_gettext("Axial direction"));
	dir->add("x");	dir->add("y");	dir->add("z");	dir->callback(style_set_cb);
	text = new Fl_Choice(210, 135, 75, 25, mgl_gettext("Text on contours"));
//	text->tooltip("Draw contour values near contour lines"));
	text->add(mgl_gettext("none"));	text->add(mgl_gettext("under"));
	text->add(mgl_gettext("above"));	text->callback(style_set_cb);
	stab->end();

	ftab = new Fl_Group(0, 25, 295, 230, mgl_gettext("Text style"));		ftab->hide();
	sc = new Fl_Check_Button(15, 40, 120, 25, mgl_gettext("Script font/style"));	sc->callback(style_set_cb);
	rm = new Fl_Check_Button(15, 70, 120, 25, mgl_gettext("Roman font"));	rm->callback(style_set_cb);
	gt = new Fl_Check_Button(15, 100, 120, 25, mgl_gettext("Gothic font"));	gt->callback(style_set_cb);
	it = new Fl_Check_Button(15, 130, 120, 25, mgl_gettext("Italic style"));	it->callback(style_set_cb);
	bf = new Fl_Check_Button(15, 160, 120, 25, mgl_gettext("Bold style"));	bf->callback(style_set_cb);
	cf = new Fl_Choice(200, 40, 80, 25, mgl_gettext("Text color"));	cf->copy(colors);cf->callback(style_set_cb);
	{ Fl_Box* o = new Fl_Box(160, 90, 120, 90, mgl_gettext("Alignment"));
		o->box(FL_DOWN_BOX);	o->align(FL_ALIGN_TOP);}
	rl = new Fl_Round_Button(170, 100, 100, 25, mgl_gettext("left"));		rl->callback(style_rdo_cb,rl);
	rc = new Fl_Round_Button(170, 125, 100, 25, mgl_gettext("at center"));	rc->callback(style_rdo_cb,rc);
	rr = new Fl_Round_Button(170, 150, 100, 25, mgl_gettext("right"));		rr->callback(style_rdo_cb,rr);
	ftab->end();

	tab->end();
	res = new Fl_Output(50, 265, 235, 25, mgl_gettext("Result"));
