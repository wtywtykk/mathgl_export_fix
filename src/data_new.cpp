...
HMDT mgl_data_subdata_ext(HCDT d, HCDT xx, HCDT yy, HCDT zz)
{
	...
	if(n*m*l>1)	// this is 2d or 3d data
	{
		...
		{
			i = long((ix?xx->vthr(i0):vx)+0.5);	if(i<0)i=0;	if(i>=nx)i=nx-1;
			j = long((iy?yy->vthr(i0):vy)+0.5);	if(j<0)j=0;	if(j>=ny)j=ny-1;
			k = long((iz?zz->vthr(i0):vz)+0.5);	if(k<0)k=0;	if(k>=nz)k=nz-1;
			r->a[i0] = d->v(i,j,k);
		}
		return r;
	}
	...
	for(i=0;i<n;i++)	for(j=0;j<m;j++)	for(k=0;k<l;k++)
	{
		...
		z = iz?long(zz->v(k)+0.5):k;	if(z<0)z=0;	if(z>=nz)z=nz-1;
