...
void MGL_EXPORT mgl_bars_xyz(HMGL gr, HCDT x, HCDT y, HCDT z, const char *pen, const char *opt)
{
	...
	if(above)	fall = false;
	mreal c1,c2;
	mreal *dd=new mreal[n], z0,zz,zp,dv=nx>n?1:0;
	if(mglchr(pen,'<'))	dv = 1;
	if(mglchr(pen,'^'))	dv = 0;
	if(mglchr(pen,'>'))	dv = -1;
	...
	for(j=0;j<m;j++)
	{
		...
		mx = j<x->GetNy() ? j:0;	my = j<y->GetNy() ? j:0;	mz = j<z->GetNy() ? j:0;
		zp = z0 = gr->GetOrgZ('x');
#pragma omp parallel for private(i,zz) ordered
		for(i=0;i<n;i++)
		{
			if(gr->Stop)	continue;
			mreal vv = x->v(i,mx), d = i<nx-1 ? x->v(i+1,mx)-vv : vv-x->v(i-1,mx);
...
void MGL_EXPORT mgl_bars_xy(HMGL gr, HCDT x, HCDT y, const char *pen, const char *opt)
{
	...
	if(above)	fall = false;
	mreal c1,c2;
	mreal *dd=new mreal[n], yy,y0,yp,dv=nx>n?1:0;
	if(mglchr(pen,'<'))	dv = 1;
	if(mglchr(pen,'^'))	dv = 0;
	if(mglchr(pen,'>'))	dv = -1;
	mreal zm = gr->AdjustZMin();
	...
	for(j=0;j<m;j++)
	{
		...
		mx = j<x->GetNy() ? j:0;	my = j<y->GetNy() ? j:0;
		yp = y0 = gr->GetOrgY('x');
#pragma omp parallel for private(i,yy) ordered
		for(i=0;i<n;i++)
		{
			if(gr->Stop)	continue;
			mreal vv = x->v(i,mx), d = i<nx-1 ? x->v(i+1,mx)-vv : vv-x->v(i-1,mx);
...
void MGL_EXPORT mgl_barh_yx(HMGL gr, HCDT y, HCDT v, const char *pen, const char *opt)
{
	...
	if(above)	fall = false;
	mreal c1,c2;
	mreal *dd=new mreal[n], xx,x0,xp,dv=ny>n?1:0;
	if(mglchr(pen,'<'))	dv = 1;
	if(mglchr(pen,'^'))	dv = 0;
	if(mglchr(pen,'>'))	dv = -1;
	mreal zm = gr->AdjustZMin();
	...
	for(j=0;j<m;j++)
	{
		...
		xp = x0 = gr->GetOrgX('y');
#pragma omp parallel for private(i,xx) ordered
		for(i=0;i<n;i++)
		{
			if(gr->Stop)	continue;
			mreal vv = y->v(i,my), d = i<ny-1 ? y->v(i+1,my)-vv : vv-y->v(i-1,my);
			mreal y1 = vv + d/2*(dv-gr->BarWidth), y2 = y1 + gr->BarWidth*d;
			vv = xx = v->v(i,mx);
			if(!above)
			{	y2 = (y2-y1)/m;		y1 += j*y2;		y2 += y1;	}
...
#pragma omp parallel for private(vv,dd,x1,x2) ordered
	for(long i=0;i<n;i++)
	{
		if(gr->Stop)	continue;
		vv = x->v(i);
		...
		gr->line_plot(n1,n2);
		n1=gr->AddPnt(mglPoint(x2,b[i+n],zVal));
		n2=gr->AddPnt(mglPoint(x2,b[i+3*n],zVal));
		gr->line_plot(n1,n2);
		n1=gr->AddPnt(mglPoint((x1+x2)/2,b[i],zVal));
		n2=gr->AddPnt(mglPoint((x1+x2)/2,b[i+n],zVal));
		gr->line_plot(n1,n2);
		n1=gr->AddPnt(mglPoint((x1+x2)/2,b[i+3*n],zVal));
		n2=gr->AddPnt(mglPoint((x1+x2)/2,b[i+4*n],zVal));
		gr->line_plot(n1,n2);
		if(sh)
#pragma omp ordered
			gr->NextColor(pal);
